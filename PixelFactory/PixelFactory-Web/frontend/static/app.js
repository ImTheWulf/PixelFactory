  ...
  assets.forEach((asset) => {
    ...
    card.innerHTML = `
      <div class="asset-thumb-wrap">
        ...
        <button class="asset-card-favorite ${asset.favorite ? "active" : ""}" type="button" title="${asset.favorite ? "Unfavorite" : "Favorite"}" aria-label="${asset.favorite ? "Unfavorite" : "Favorite"} ${escapeHtml(displayName)}">${asset.favorite ? "★" : "☆"}</button>
    ...
    card.querySelector(".asset-card-favorite")?.addEventListener("click", async (event) => {
      event.stopPropagation();
